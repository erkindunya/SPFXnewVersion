<template>
    ...
    <div v-if="hasHardware">
        <div v-for="(items, key) in hardware" :key="key">
            ...
            <ul v-if="items.length > 0">
                <li v-for="(item, itemKey) in items" :key="itemKey">{{item.name}} - {{item.price}} GBP</li>
            </ul>
    ...
    <div class="row">
        ...
        <div class="col text-right">
            <button type="button" class="btn btn-primary" v-on:click="submit" :disabled="$v.$invalid">Submit</button>
        </div>
    </div>
</div>
</template>
